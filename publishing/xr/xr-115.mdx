...
sidebarTitle: "XR-115 User & Controller Changes"
description: "Titles supporting multiple users must handle controller addition/removal and user addition/removal during gameplay, preserving state and providing a path back to gameplay."
icon: "user-plus"
keywords: ["XR-115", "controller addition", "controller removal", "user addition", "user removal", "XUserAddAsync", "XUserFindControllerForUserWithUiAsync", "XUserDeviceAssociationChangedCallback", "multiplayer"]
---
...
**Controller Addition:** After titles have selected or have been provided the initial user and controller, titles can optionally accept input from other controllers. Titles which support multiplayer experiences should consider how an additional player or controller is added into gameplay and is bound to a user with `XUserAddAsync`. For example, 'Press A to join' or illustrating a controller silhouette on a player select screen.
...
**User Addition:** When using ERA or UWP, if a new user signs in, the title can add the user to the title if the title supports a multi-user experience, but the title must not automatically switch active users to the new user or interrupt the active users' experience based on a new signed-in user event.
...
**Additional/Secondary User Removal:** Titles can handle this event at their discretion, usually by removing the player from gameplay immediately or prompting the player to rejoin in the context of the multiplayer experience being used.
