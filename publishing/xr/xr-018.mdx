...
sidebarTitle: "XR-018 UGC"
description: "Titles that include user-generated content must provide reporting, publish content guidelines, respect UGC privileges, and moderate mods where required."
icon: "user-pen"
keywords: ["XR-018", "UGC", "user generated content", "mods", "text filtering", "StringService", "moderation", "reporting"]
---
...
Titles that use the [StringService](https://learn.microsoft.com/dotnet/api/microsoft.xbox.services.system.stringservice) API meet the proactive detection requirement (for text scenarios) automatically. Titles should default to leveraging the StringService API for names and other short static text strings. If titles have other text scenarios that allow lengthier text input, or if there are technical or other limiting circumstances, titles can choose to implement their own variant of text filtering by restricting a set of terms from being displayed to another player on Xbox services. Refer to the Forbidden Terms List that must be leveraged as a minimum baseline for blocking or obfuscation from non-local players on Xbox services.
...
<Note>
  **Additional Related Xbox Requirements (XRs)**
  ...
  - [XR-045: Xbox network and Account Privileges](/publishing/xr/xr-045): Ensure proper privilege checks occur.
  - Console game only, [XR-007: Cross-Network Play, Data Usage, and Companion App Interactions](/publishing/xr/xr-007): If your title supports cross-network play, ensure the title is compliant with UGC requirements outlined in XR-007.
</Note>
...
- Scenarios in which players can minimally customize developer-managed content within strictly established parameters.
  - Exempt example: Changing the color of a player emblem within a range of provided options that do not substantively modify the image.
  - Non-exempt example: Layering, rotating, cropping, or otherwise modifying a player emblem in a way that enables the creation of new imagery.
...
- Content created in an online setting that is transient, does not produce a tangible UGC artifact (photo, text, custom save file, mod package file, etc.), and is not intended by the developer to function as player content.
  - Exempt examples: Drawings or words made from bullet holes; using game physics to nudge objects in a particular formation.
  - Non-exempt example: Placing objects through a map editor or other developer-supported customization feature.
...
Xbox gamertags are exempt from UGC requirements and should not be subject to text filtration, title-managed report options, or obfuscation due to a restricted UGC privilege. Players can report Gamertags to Xbox directly by accessing the user's gamercard via [XR-047: User Profile Access](/publishing/xr/xr-047).
...
3. If there is no way to report inappropriate content, in each area, enter a string, sub string, etc. that is in the Forbidden Terms List.
