...
sidebarTitle: "XR-064 Joinable Sessions"
description: "Titles offering joinable game sessions must enable joinability through the Xbox shell interface (and Game Bar for PC cross-platform), keeping Multiplayer Activity state accurate throughout the session."
icon: "users-viewfinder"
keywords: ["XR-064", "joinable sessions", "Xbox shell", "Game Bar", "Multiplayer Activity", "invites", "cross-platform"]
---
...
For information on managing game sessions using the GDK, read the Multiplayer overview in the GDK documentation.
...
2. Using Profile B not running the title, access the Friends List via the Xbox shell interface and verify that Profile A is listed as joinable.
   - It is possible that some games may only support private multiplayer sessions and therefore not appear as joinable for Profile B, in which case jump to step 6.
3. Profile B joins Profile A's game.
...
3. Using Profile B, access the Friends List via the Xbox shell interface and select to join Profile A's game.
   - It is possible that some games may only support private multiplayer sessions and therefore do not appear as joinable for Profile B, in which case jump to step 5.
4. Confirm that both profiles are placed into the game experience together.
