...
sidebarTitle: "XR-037 Content Dependencies"
description: "Add-on content must not be required to complete base game features, must not depend on other add-ons, and must gracefully handle missing content when loading save data."
icon: "boxes-stacked"
keywords: ["XR-037", "content packages", "DLC", "add-on", "save game dependencies", "multiplayer DLC"]
---
...
2. After creating a save within an area provided by the content package and then deleting the content package, the game clearly messages the user indicating that content _x_ needs to be installed to load the game save. Titles can also query to see if the user has an entitlement to that content and either:
