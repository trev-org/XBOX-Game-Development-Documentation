...
description: "Titles that support Intelligent Delivery must handle scenarios gracefully when content that is not currently installed is needed, using PackageInstallChunksAsync or AddChunkSpecifiersAsync."
icon: "truck-fast"
keywords: ["XR-129", "Intelligent Delivery", "PackageInstallChunksAsync", "AddChunkSpecifiersAsync", "language packs", "on-demand content", "streaming install", "Features and Recipes"]
---
...
3. The title remains stable and does not crash or become unresponsive.
...
3. The title crashes or becomes unresponsive.
...
2. The title remains stable and does not crash or become unresponsive.

**Fail Examples**

1. The title does not install the expected device-specific content.
2. The title crashes or becomes unresponsive.
...
10. Repeat steps 5\\u20139 on an Xbox Series X|S.
...
1. The title crashes or becomes unresponsive.
...
1. The title crashes or becomes unresponsive when optional content is not installed.
