...
description: "Titles must not store users' profile information sourced from the Xbox network beyond a locally stored cache used for offline scenarios, and must refresh caches on the next available connection."
icon: "user-gear"
keywords: ["XR-048", "profile settings", "gamertag caching", "profile data", "source of truth"]
---
...
2. The title does not update a user's gamertag for persistent posts, such as game clips, replays, leaderboards, or other custom posts, such as messages, bulletin posts, user challenges, costumes, themes, livery, tournaments, and league. This applies to both new and previously created posts.
