...
sidebarTitle: "XR-074 Connectivity Loss"
description: "Titles must gracefully handle loss of connectivity to Xbox and partner services, honor Xbox retry policies, and clearly communicate the source of any service issue to users."
icon: "plug-circle-xmark"
keywords: ["XR-074", "connectivity", "partner services", "Xbox services", "retry policy", "RequireXboxLive", "low bandwidth", "suspend resume", "xbstress", "Fiddler"]
---
...
If a service is unavailable, titles are permitted to retry their request, but they can do so only within the parameters allowed by Xbox. For more information about service resiliency for Xbox console titles, including required HTTP retry and back-off logic, as well as user-interaction guidance for individual services, see the Service Interruption Resiliency for Titles documentation in the GDK.
...
For information on networking using the GDK, read the Networking section of the GDK documentation.
...
2. With no connection to Xbox but retaining a local network connection, launch the title.
   - If utilizing an Ethernet switch/hub, disconnect the uplink cable from the network device.
   - If the device is connected via Wi-Fi, disconnect the uplink cable from the wireless access point.
...
Titles must not crash or cause user data loss when a user's internet connection drops below 192 Kbps.
...
1. Disable all network restrictions in `xbstress.exe`.
2. Perform title-related online actions, including but not limited to:
   - Navigating all menus.
   - Playing an online game session.
3. While performing Step 2, enable the `xbstress` minimum profile.
...
- On Console, set up Fiddler to debug web service calls.
- On PC, set up Fiddler to debug web service calls.
...
<Tip>
  **Steps to configure the block list:**
  ...
  2. In Fiddler, identify which hosts are non-Microsoft services:
     - Sort the list of sessions by Host and find hosts that DO NOT contain any of the following: `microsoft, msft, xboxlive, xboxservices, live, PlayFabApi, msn, bing`.
