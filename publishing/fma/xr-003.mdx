...
description: "FMA scenario severities for XR-003 Title Quality for Submission — Title Integrity, Save-Game Compatibility, Headset State Change, and Headset State Change after Suspend."
icon: "broom"
keywords: ["FMA XR-003", "title integrity", "save-game compatibility", "PLS", "headset state change", "DLC", "localization"]
---
...
See the [XR-003 reference](/publishing/xr/xr-003) for the underlying certification requirement.
...
| The title displays null characters within a user-generated string of text when customized to contain special characters (for example, subscript and superscript characters). _Note:_ Use XR-046 for Display Name issues. | Not Relevant (-4) | Xbox One, Xbox Series, Windows |
...
| \[DLC\] A specific piece of DLC doesn't work in-title as expected, however other examples of that type of DLC do work as expected; for example, one skin pack doesn't work, at least one other does. | Moderate (6) | Xbox One, Xbox Series, Windows |
...
| \[DLC\] A particular type of DLC as a whole doesn't work in-title as expected; for example, all consumables or all skin packs. | Critical (12) | Xbox One, Xbox Series, Windows |
| \[Cross Region (Multiplayer)\] The title supports cross-region multiplayer gameplay over the Xbox network but users on different SKUs can't enter gameplay sessions together. | Critical (12) | Xbox One, Xbox Series, Windows |
...
- For digital only titles, users can't experience the scenario of trying to load a content-updated save on a previous version of the title as they can never revert back to it.
- PLS — check the `AppxManifest.xml` to verify that PLS is being allocated for the submission. What they use it for isn't as important as long as other game-save data is saved using connected storage and is available on another console. Example pulled from `AppxManifest.xml`:
  ```xml
