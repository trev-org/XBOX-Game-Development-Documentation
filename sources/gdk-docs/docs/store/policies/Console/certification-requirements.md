...
Xbox Requirements (XRs) consist of the policies, technical requirements, and product component-related requirements to which all developers and publishers of Xbox console titles must conform. XRs ensure that products created for Xbox consoles aren't only stable and reliable but also provide a user experience that's consistent, safe, secure, and enjoyable.
...
* Support navigation via gamepad input. Titles may require other peripherals for use with prior approval.
...
* Between generations (Xbox One and Xbox Series X\|S), games that share the same TitleID must:
  ...
  * Though not required, it's recommended that scenarios that support multiplayer, cooperative, or competitive experiences offer at least one shared matchmaking hopper and allow for cross generation invite and join for shared content experiences.
...
All aspects of your app should accurately describe the functions, features, and any important limitations of your app, including required or supported input devices. Your app may not use a name or icon similar to that of other apps. Your app may not represent itself as being from a company, government body, or other entity without permission to do so.
...
Titles may choose to share or roam bundles of in-game items across gaming networks, provided those bundles are sold through the Microsoft Store and managed by the title. This includes virtual currency and items unlocked with virtual currency.
...
Titles must not:
  ...
  * offer such content for purchase with products or experiences that don't have the same features and capabilities as the game for Xbox consoles. Player progression and game data can be shared with such products or experiences (for example, "companion experiences").

If your product violates this policy, it may lose eligibility for editorial placement, store marketing, and promotions.
...
Titles can, at their discretion, enable synchronous or asynchronous cross-network gameplay among Xbox network users and users of non-Xbox gaming networks subject to the following restrictions:
  ...
  * Titles must visually identify Xbox network users when they're playing with players from non-Xbox gaming networks.
...
On Xbox consoles, titles must not display images or refer to components of the console system or components of peripherals using terms that aren't included in the terminology list.
...
Titles must be compliant with Windows Store policy regarding App Security. Xbox also recommends that exclusive OS games use the **/GS** flag, but this practice isn't enforced due to performance implications with some design patterns.
...
Your app must not attempt to change or extend the described functionality through any form of dynamic inclusion of code that's in violation of Store Policies. Your app shouldn't, for example, download a remote script and then execute that script in a manner that isn't consistent with the described functionality.
...
Your app may contain fully integrated middleware (such as third-party cross-platform engines and third-party analytics services), but must not deliver or install non-integrated third-party owned or branded apps or modules unless they're delivered as in-app products.
...
Note: "Semi-annual" means January-June, July-December. Base game achievements and Gamerscore don't count towards the semi-annual limits.
...
After an achievement is published to users, it can't be removed, nor can its unlock rules or rewards be changed. Achievement text strings (name, description) or art (icons/background) can be modified.
...
-----|------------------|-------------------
July 1, 2026 | Console: 16.3 | Removed **XR-083 Submission Validator** and moved the requirement to [XR-003 Title Quality for Submission](../XR/XR003.md). There's no test case for it as a non-tested XR. If an Exception is required for a Submission Validator fail, log it under XR-003.<br><br>Re-added test case [052-05 Correct User Association](console-certification-requirements-and-tests.md) to the console XR and Test Cases page as it was accidentally removed in the last update.